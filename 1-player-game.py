...
def draw_move(board):
    ...
    # Make computer move and place X in board array
    computerMove = randrange(10)
    while computerMove not in movesAvailable:
        computerMove = randrange(10)
    for i in range(rows):
        for j in range(cols):
            if board[i][j] == computerMove:
                board[i][j] = 'X'
    
    # Place computer move on layout for output
    global layout
    for character in layout:
        if character == str(computerMove):
